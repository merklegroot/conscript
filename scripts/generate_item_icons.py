...
def render_molotov_lit(out_path: str) -> None:
    seed = 5011
    bg = _textured_bg(seed)

    # Bottle mask (reuse molotov proportions)
    mask = Image.new("L", (W, H), 0)
    d = ImageDraw.Draw(mask)
    bx0, by0 = int(W * 0.43), int(H * 0.17)
    bx1, by1 = int(W * 0.57), int(H * 0.84)
    ...
    # soot at tip (charred + now lit)
    soot = Image.new("L", (W, H), 0)
    sdd = ImageDraw.Draw(soot)
    sdd.ellipse((cx - 90, rag_top - 10, cx + 90, rag_top + 90), fill=110)
    soot = soot.filter(ImageFilter.GaussianBlur(26))
    soot = ImageChops.multiply(soot, rag_mask)
    rag = ImageChops.subtract(rag, Image.merge("RGB", (soot, soot, soot)))

    # rim shadow where rag enters bottle
    entry_shadow = Image.new("L", (W, H), 0)
    es = ImageDraw.Draw(entry_shadow)
    es.ellipse((cx - 46, by0 + 52, cx + 46, by0 + 92), fill=70)
    entry_shadow = entry_shadow.filter(ImageFilter.GaussianBlur(10))
    ...
    # Flame (small, at rag tip) + warm glow
    flame = Image.new("RGB", (W, H), (0, 0, 0))
    flame_mask = Image.new("L", (W, H), 0)
    fd = ImageDraw.Draw(flame_mask)
    tip_y = rag_top + 18
    # core
    fd.ellipse((cx - 10, tip_y - 30, cx + 10, tip_y + 4), fill=220)
    fd.ellipse((cx - 18, tip_y - 46, cx + 18, tip_y + 6), fill=140)
    fd.polygon([(cx, tip_y - 64), (cx - 18, tip_y - 26), (cx + 18, tip_y - 26)], fill=120)
    flame_mask = flame_mask.filter(ImageFilter.GaussianBlur(6))

    # colorize flame by layering warm colors
    warm1 = Image.new("RGB", (W, H), (255, 190, 80))
    warm2 = Image.new("RGB", (W, H), (255, 120, 30))
    warm3 = Image.new("RGB", (W, H), (255, 240, 200))
    flame = Image.composite(warm1, flame, flame_mask)
    inner = flame_mask.point(lambda p: int(p * 0.55))
    flame = Image.composite(warm2, flame, inner.filter(ImageFilter.GaussianBlur(4)))
    core = flame_mask.point(lambda p: int(p * 0.25))
    flame = Image.composite(warm3, flame, core.filter(ImageFilter.GaussianBlur(3)))
    bg = _screen_blend(bg, flame, 0.9)

    # glow spill (warm tint) around the top of bottle
    glow = Image.new("RGB", (W, H), (255, 120, 40))
    glow_mask = Image.new("L", (W, H), 0)
    gd = ImageDraw.Draw(glow_mask)
    gd.ellipse((cx - 220, tip_y - 220, cx + 220, tip_y + 160), fill=120)
    glow_mask = ImageChops.multiply(glow_mask.filter(ImageFilter.GaussianBlur(60)), mask)
    glow_layer = _paste_with_mask(Image.new("RGB", (W, H), (0, 0, 0)), glow, glow_mask)
    bg = _screen_blend(bg, glow_layer, 0.35)
