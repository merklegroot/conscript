...
internal sealed class GasGaugeViewerDialog
{
    ...
    public void Draw(Font font, Texture2D faceTexture, Texture2D needleTexture, int level, int screenWidth, int screenHeight)
    {
        ...
        if (needleTexture.Id != 0)
            DrawNeedle(needleTexture, level);
    ...
    private void DrawNeedle(Texture2D needleTexture, int level)
    {
        float needleAspect = needleTexture.Width / (float)needleTexture.Height;
        float needleH = _faceRect.Height * 0.44f;
        float needleW = needleH * needleAspect;
        float pivotX = needleW / 2f;
        float pivotY = needleH * GasGaugeCatalog.NeedlePivotYRatio;

        float gaugeCenterX = _faceRect.X + _faceRect.Width / 2f;
        float gaugeCenterY = _faceRect.Y + _faceRect.Height / 2f;
        ...
        var needleSrc = new Rectangle(0, 0, needleTexture.Width, needleTexture.Height);
        float rotation = GasGaugeCatalog.GetNeedleRotation(level);
